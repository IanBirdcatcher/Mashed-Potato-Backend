...
exports.login = async (req, res) => {
  ...
  if (user.userId === undefined) {
    ...
    user.lName = lastName;
    console.log(user);
    await User.update(user, { where: { id: user.userId } })
      .then((num) => {
        if (num == 1) {
          console.log("updated user's name");
        } else {
          console.log(
            `Cannot update User with id=${user.userId}. Maybe User was not found or req.body is empty!`
          );
        }
      })
      .catch((err) => {
        console.log("Error updating User with id=" + user.userId + " " + err);
      });
  }
